...
if (!isset($opc_address_values) || !is_array($opc_address_values)) {
    trigger_error("Missing or invalid opc_address_values array:" . var_export($opc_address_values, true), E_USER_ERROR);
}

if (ACCOUNT_GENDER == 'true') {
    $field_name = "gender[$opc_address_type]";
    $male_id = "gender-male-$opc_address_type";
    $female_id = "gender-femail-$opc_address_type";
    echo zen_draw_radio_field ($field_name, 'm', ($address_values['bill']['gender'] == 'm'), "id=\"$male_id\"") . 
    "<label class=\"radioButtonLabel\" for=\"$male_id\">" . MALE . '</label>' . 
    zen_draw_radio_field ($field_name, 'f', ($address_values['bill']['gender'] == 'f'), "id=\"$female_id\"") . 
...
?>
      ...
      <?php echo zen_draw_input_field ($field_name, $opc_address_values['firstname'], zen_set_field_length(TABLE_CUSTOMERS, 'customers_firstname', '40') . " id=\"$field_id\"") . 
...
?>
      ...
      <?php echo zen_draw_input_field($field_name, $opc_address_values['lastname'], zen_set_field_length(TABLE_CUSTOMERS, 'customers_lastname', '40') . " id=\"$field_id\"") . 
...
?>
      ...
      <?php echo zen_draw_input_field($field_name, $opc_address_values['company'], zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_company', '40') . " id=\"$field_id\"") . 
...
?>
      <label class="inputLabel" for="<?php echo $field_id; ?>"><?php echo ENTRY_STREET_ADDRESS; ?></label>
        <?php echo zen_draw_input_field($field_name, $opc_address_values['street_address'], zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_street_address', '40') . " id=\"$field_id\"") . 
...
?>
      ...
      <?php echo zen_draw_input_field($field_name, $opc_address_values['suburb'], zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_suburb', '40') . " id=\"$field_id\"") . 
...
?>     
      ...
      <?php echo zen_draw_input_field($field_name, $opc_address_values['city'], zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_city', '40') . " id=\"$field_id\"") . 
...
if (ACCOUNT_STATE == 'true') {
   ...
   $state_field_label = ($opc_address_values['show_pulldown_states']) ? '' : ENTRY_STATE; 
...
<?php
        echo zen_draw_pull_down_menu($zone_field_name, zen_prepare_country_zones_pull_down($opc_address_values['country_id'], $opc_address_values['zone_id']), $opc_address_values['zone_id'], "id=\"$state_zone_id\"");
...
?>
      <label class="inputLabel" for="<?php echo $state_field_id; ?>" id="<?php echo $state_label_id; ?>"><?php echo $opc_address_values['state_field_label']; ?></label>
<?php
    echo zen_draw_input_field($state_field_name, $opc_address_values['state'], zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_state', '40') . " id=\"$state_field_id\"");
    if (zen_not_null(ENTRY_STATE_TEXT)) {
        echo "&nbsp;<span class=\"alert\" id=\"$state_text_id\">" . ENTRY_STATE_TEXT . '</span>';
    }
    if (!$opc_address_values['show_pulldown_states']) {
        echo zen_draw_hidden_field($zone_field_name, $opc_address_values['zone_name'], "id=\"$state_zone_id\"");
    }
?>
      <br class="clearBoth" />
<?php
...
?>
      ...
      <?php echo zen_draw_input_field($field_name, $opc_address_values['postcode'], zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_postcode', '40') . " id=\"$field_id\"") . 
...
?>
      ...
      <?php echo zen_get_country_list($field_name, $opc_address_values['country_id'], "id=\"$field_id\" " . ($opc_address_values['show_pulldown_states'] ? ('onchange="update_zone(this.form, \'' . $opc_address_type . '\');"') : '')) . 
